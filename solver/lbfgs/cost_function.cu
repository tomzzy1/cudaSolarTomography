...
void cpu_cost_function::f_gradf(const floatdouble *d_x, floatdouble *d_f, floatdouble *d_gradf)
{
	// Copy device x to host memory
	CudaSafeCall( cudaMemcpy(m_h_x, d_x, m_numDimensions * sizeof(floatdouble), cudaMemcpyDeviceToHost) );
	floatdouble h_f;
	...
	CudaSafeCall( cudaMemcpy(d_f,         &h_f,                   sizeof(floatdouble), cudaMemcpyHostToDevice) );
	CudaSafeCall( cudaMemcpy(d_gradf, m_h_gradf, m_numDimensions * sizeof(floatdouble), cudaMemcpyHostToDevice) );
}
